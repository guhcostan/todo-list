...
const getTodoByName = async (tableName: string) => {
    const todosData = await axios.get(
        process.env.REACT_APP_API_URL + '/todo/?name=' + tableName
    )
...
const createItem = (tableId: number, todo: Todo, newItem: string) => {
    return axios.post(
        process.env.REACT_APP_API_URL + '/todo/itens/' + tableId,
        { id: Math.random(), item: newItem }
    )
}
...
const deleteItem = (tableId: number, itemId: number) => {
    return axios.delete(
        process.env.REACT_APP_API_URL + '/todo/itens/' + tableId + '/' + itemId
    )
}
